...
-- Modify compression policy to keep more historical data uncompressed
-- Only compress data older than 30 days
SELECT add_compression_policy('"OHLCV"', INTERVAL '30 days');

-- Configure compression settings to optimize compression ratio while maintaining query performance
...
-- Create continuous aggregates for common timeframes with longer retention
...
-- Create refresh policies for continuous aggregates with longer retention
...
-- Create a function to maintain the hypertable and refresh materialized views
